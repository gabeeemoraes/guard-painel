...
export default function Configuracoes() {
  ...
  // Gestão do segundo usuário (somente admin)
  ...
  useEffect(() => {
    if (isAdmin) {
      api.get<{ users: ManagedUser[] }>("/users").then((res) => setUsers(res.users));
    }
    api.get<{ storeName: string }>("/settings").then((res) => setStoreName(res.storeName));
  }, [isAdmin]);
  ...
  async function saveStoreName() {
    ...
    try {
      await api.patch("/settings", { storeName });
      notify("Nome da loja atualizado.", "success");
    } catch (err: any) {
        ...
        <div className="grid grid-cols-2 mb-4">
          ...
          {isAdmin && (
            <div className="card">
              ...
              <button className="btn btn-primary" onClick={saveStoreName} disabled={savingStoreName}>
          ...
          <div className="card">
                ...
                <div key={u.id} style={{ borderBottom: "1px solid var(--border-color)", paddingBottom: 16, marginBottom: 16 }}>
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex gap-2 items-center">
                      ...
                      <button className="btn btn-secondary" onClick={() => resetUserPassword(u)}>
                        Redefinir senha
                      </button>
                    </div>
                  </div>
            ...
            {users.filter((u) => u.role === "USER").length === 0 && (
              <p className="text-secondary" style={{ fontSize: 13 }}>
                Nenhum usuário comum configurado. Defina SECONDARY_USER_EMAIL e SECONDARY_USER_PASSWORD no .env e rode
                o script de seed novamente.
              </p>
            )}
          </div>
        )}
