...
export default function Dashboard(){
 ...
 if(!data)return null;
 const kpis=[{label:"Vendas Totais",value:formatCurrency(data.grossRevenue),icon:DollarSign},{label:"Pedidos",value:data.ordersCount.toLocaleString("pt-BR"),icon:ShoppingCart},{label:"Ticket Médio",value:formatCurrency(data.averageTicket),icon:Tag},{label:"Produtos Ativos",value:data.productsCount.toLocaleString("pt-BR"),icon:Package}];
 return <><Header title="Dashboard"/><main className="content dashboard-content dashboard-v2">
  <section className="dashboard-toolbar"><div><h2>Dashboard</h2><p>Acompanhe o desempenho das suas lojas integradas em um só lugar.</p></div><label className="dashboard-date-select"><CalendarDays size={16}/><select value={range} onChange={e=>setRange(e.target.value)}><option value="today">Hoje</option><option value="last7">Últimos 7 dias</option><option value="last30">Últimos 30 dias</option><option value="currentMonth">Mês atual</option></select><ChevronDown size={15}/></label></section>
  <section className="integration-strip">{(["shopee","mercadolivre","tiktokshop"] as MarketplaceProvider[]).map(provider=>{const item=integrations.find(i=>i.provider===provider);return <button type="button" className={`integration-card ${item?.connected?"connected":""}`} key={provider} onClick={()=>navigate(`/integracoes?provider=${provider}`)}><MarketplaceMark provider={provider}/><div><strong>{META[provider].label}</strong><span className={item?.connected?"status-connected":"status-offline"}><i/>{item?.connected?`Loja conectada${item.shopName?` · ${item.shopName}`:""}`:"Não conectada"}</span></div><ChevronDown size={18} className="integration-arrow"/></button>})}<button type="button" className="integration-card add-store" onClick={()=>navigate("/integracoes")}><span className="add-store-icon"><Plus size={21}/></span><div><strong>Conectar nova loja</strong><span>Expanda suas vendas</span></div></button></section>
  <section className="kpi-grid">{kpis.map(({label,value,icon:Icon})=><div className="dashboard-kpi" key={label}><div className="kpi-top"><span>{label}</span><span className="kpi-icon"><Icon size={18}/></span></div><strong className="kpi-value-box">{value}</strong><div className="kpi-trend"><span>Dados reais sincronizados</span></div></div>)}</section>
  <section className="dashboard-main-grid"><div className="dashboard-card sales-card"><div className="dashboard-card-head"><div><h3>Vendas no período</h3><div className="chart-legend"><span><i className="legend-dot shopee"/>Consolidado</span></div></div><div className="chart-range-tabs"><button className={range==="last7"?"active":""} onClick={()=>setRange("last7")}>7 dias</button><button className={range==="last30"?"active":""} onClick={()=>setRange("last30")}>30 dias</button><button className={range==="currentMonth"?"active":""} onClick={()=>setRange("currentMonth")}>Mês</button></div></div><SalesChart data={data.salesEvolution}/></div>
   <div className="dashboard-card distribution-card"><div className="dashboard-card-head"><h3>Vendas por marketplace</h3><button className="icon-ghost" type="button" onClick={()=>navigate("/faturamento")} aria-label="Ver faturamento"><MoreHorizontal size={17}/></button></div><div className="donut-wrap"><div className="donut" style={{background:donutGradient}}><div><strong>{formatCurrency(total)}</strong><span>Total</span></div></div><div className="donut-legend">{marketplaces.map(m=>{const pct=total?(Number(m.grossRevenue)/total)*100:0;return <div key={m.provider}><span><i className={`legend-dot ${META[m.provider].className}`}/>{META[m.provider].label}</span><strong>{formatCurrency(m.grossRevenue)} <small>{pct.toFixed(1)}%</small></strong></div>})}</div></div></div></section>
  <section className="dashboard-bottom-grid"><div className="dashboard-card orders-card"><div className="dashboard-card-head"><h3>Pedidos recentes</h3><button className="small-link" type="button" onClick={()=>navigate("/pedidos")}>Ver todos</button></div>{recentOrders.length?<div className="recent-orders"><div className="recent-order-head"><span>#</span><span>Marketplace</span><span>Status</span><span>Valor</span><span>Data</span></div>{recentOrders.map(o=><div className="recent-order" key={`${o.provider}-${o.id}`}><strong>#{o.id}</strong><span className="order-market"><MarketplaceMark provider={o.provider}/>{META[o.provider].label}</span><span className="order-status"><i/>{o.status}</span><strong>{formatCurrency(o.value)}</strong><span>{new Date(o.date).toLocaleDateString("pt-BR")} <small>{new Date(o.date).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}</small></span></div>)}</div>:<div className="empty-inline">Nenhum pedido no período.</div>}</div>
