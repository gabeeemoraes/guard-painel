...
export default function Dashboard(){
 ...
 const netAdjustments=Number(data.grossRevenue||0)-Number(data.netRevenue||0);
 const kpis=[
   {label:"Faturamento Bruto",value:data.grossRevenue,format:formatCurrency,icon:DollarSign},
   {label:"Receita Líquida",value:data.netRevenue,format:formatCurrency,icon:ArrowUpRight},
   {label:"Lucro",value:data.profit,format:formatCurrency,icon:Tag},
   {label:"Margem",value:data.margin,format:(v:number)=>`${Number(v||0).toFixed(1).replace(".",",")}%`,icon:Package}
 ];
 ...
 return <><Header title="Dashboard"/><main className="content dashboard-content dashboard-v2" data-testid="dashboard-page">
  ...
  <section className="dashboard-card financial-overview"><div className="dashboard-card-head"><div><h3>Visão financeira</h3><p className="financial-subtitle">Entenda como o faturamento chega ao lucro.</p></div><button className="small-link" type="button" onClick={()=>navigate("/faturamento")}>Ver faturamento</button></div><div className="financial-flow"><div className="financial-step"><span>Faturamento bruto</span><strong>{formatCurrency(data.grossRevenue)}</strong></div><div className="financial-connector">−</div><div className="financial-deductions"><div><span>Taxas</span><strong>{formatCurrency(data.totalFees)}</strong></div><div><span>Descontos</span><strong>{formatCurrency(data.totalDiscounts)}</strong></div><div><span>Frete</span><strong>{formatCurrency(data.totalShipping)}</strong></div></div><div className="financial-connector">=</div><div className="financial-step net"><span>Receita líquida</span><strong>{formatCurrency(data.netRevenue)}</strong></div><div className="financial-connector">−</div><div className="financial-step"><span>Custos contabilizados</span><strong>{formatCurrency(data.totalCosts)}</strong></div><div className="financial-connector">=</div><div className="financial-step profit"><span>Lucro</span><strong>{formatCurrency(data.profit)}</strong><small>Margem {Number(data.margin||0).toFixed(1).replace(".",",")}%</small></div></div><div className="financial-note">Diferença entre bruto e líquido: <strong>{formatCurrency(netAdjustments)}</strong>. O lucro considera os custos contabilizados pelo sistema.</div></section>
