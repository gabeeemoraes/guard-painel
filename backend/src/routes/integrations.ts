...
router.get("/:provider/callback", async (req,res)=>{
  ...
  try {
    ...
    if(!oauthToken) throw new Error("Sessão de conexão expirada. Inicie a conexão novamente.");
    const payload=jwt.verify(oauthToken,env.sessionSecret) as {userId?:string;provider?:string};
    if(!payload.userId || payload.provider!==provider) throw new Error("Estado OAuth inválido.");
    if(provider!=="shopee"){
    ...
    await logAudit(payload.userId,`${provider}_connected`); return res.redirect(`${env.frontendUrl}/integracoes?success=1&provider=${provider}`);
